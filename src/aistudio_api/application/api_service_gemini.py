...
async def handle_gemini_generate_content(
    ...
    *,
    ...
    raise HTTPException(
        429, detail={"message": str(last_error), "type": "rate_limit_exceeded"}
    ) from last_error
    ...
    async def stream_response():
        ...
        if busy_lock is None:
            yield (
                "data: "
                + json.dumps(
                    {"error": {"message": "Server not ready"}}, ensure_ascii=False
                )
                + "\n\n"
            )
            return
        ...
        async with busy_lock:
            ...
            try:
                ...
                yield (
                    "data: "
                    + json.dumps({"error": {"message": str(exc)}}, ensure_ascii=False)
                    + "\n\n"
                )
            finally:
                if normalized is not None:
