...
- Multi-account Cookie intelligent rotation scheduling with per-model independent cooldowns (`sticky`, `round_robin`, `lru`, `least_rl`)
...
Memory footprint reference (measured): pure-Python service layer runs at ~30–85 MB; with the bundled CloakBrowser (Chromium) enabled, the full daemon idles at ~500–650 MB. On Termux make sure the device has at least 1 GB of free RAM.
...
- **Account Management**: Support for importing Google cookies, automatic probe for multi-login accounts (`u/0`, `u/1`...), per-model rate limit and cooldown visibility, manual activation, renaming, and secure deletion.
- **Rotation Policies**: Four rotation strategies:
